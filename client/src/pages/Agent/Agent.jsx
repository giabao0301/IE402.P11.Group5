  ...
  return (
    <div className="agentsPage">
      <div className="hero">
        ...
        <p>Chọn từ hơn 1000+ môi giới bất động sản tại để mua, bán hoặc cho thuê nhanh hơn và dễ dàng hơn.</p>
      </div>
            ...
            {featuredAgents.hcm.map(agent => (
              <div className="agentCard" key={agent.id}>
                <div className="agentInfo">
                  <img src={agent.image} alt={agent.name} className="agentPhoto" />
                  <img src={agent.logo} alt={agent.company} className="companyLogo" />
                  <h4>{agent.name}</h4>
                  <p>{agent.company}</p>
